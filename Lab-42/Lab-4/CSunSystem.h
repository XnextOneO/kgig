...
private:
	...
	CMatrix MaCoords;
	CMatrix  JCoords;
	CMatrix SputnikCoords;
	CMatrix SputnikCoords1;
	...
	double wEarth;
	double VSputnik;// Угловая скорость палнет
	double wMoon;
	double wVenus;
	double wMercury;
	double wMars;
...
CSunSystem::CSunSystem()
{
	...
	double RoE = 7.5 * rS, RoM = 2 * rE,              // Радиусы орбит
		RoV = 6 * rV, RoMe = 8 * rMe,
		RoMa = 21 * rMa, RoJ = 17 * rJ, RoSp = 2 * rSp;
	...
	RoE = (EarthOrbit.right - EarthOrbit.left) / 2;  // радиусы орбит
